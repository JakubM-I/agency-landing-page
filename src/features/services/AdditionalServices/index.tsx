...
const AdditionalServices: React.FC<SectionAdditionalServiceProps> = ({ items, images }) => {
    return (
        <section className="mt-16 sx:mt-[67px] py-16 sx:py-[67px] bg-[#F5EDE0]" aria-labelledby="additionalServices">
            <SectionTitle title="Additional Services" styling="mb-8" id="additionalServices" />
            <div className="flex flex-col sm:flex-row flex-nowrap sm:flex-wrap md:flex-nowrap justify-center items-stretch gap-10 sm:gap-8 px-6">
                {items.map((item, index) => (
                    <article key={index} className="flex flex-col justify-start items-center gap-4 sm:flex-[0_1_45%] md:flex-[0_1_26%] p-6 bg-white h-auto rounded-2xl shadow-xl">
